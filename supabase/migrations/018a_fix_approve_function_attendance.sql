-- Fix approve_team_vs_team_invite function to not require created_at/updated_at in INSERT
-- The columns have DEFAULT values, so we don't need to specify them

DROP FUNCTION IF EXISTS approve_team_vs_team_invite(UUID, UUID, UUID, UUID);
